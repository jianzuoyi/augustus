	...
	with open('all.sh', 'w') as out:
		for k, v in dict.iteritems():
			with open(k + '.sh', 'w') as each:
				cmd = '''date &&
cd {path}/../01.bax2bam &&
/its1/GB_BT1/lixiangfeng/software/smrtlink/smrtcmds/bin/bax2bam -o {k} {v1} {v2} {v3} &&
cd {path}/../02.CCS &&
/its1/GB_BT1/lixiangfeng/software/smrtlink/smrtcmds/bin/ccs --numThreads 25 --noPolish --minLength 100 --maxLength 15000 --minPasses 0 --minPredictedAccuracy 0.85 --minZScore -999 --maxDropFraction 0.8 --minSnr 3.75 --reportFile {k}_report.txt {path}/../01.bax2bam/{k}.subreads.bam {k}.ccs.bam &&
date
